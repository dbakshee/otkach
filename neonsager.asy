...
xaxis("$k_x$", YEquals(g*sqrt(3)/2), xmin=-1g, xmax=-0.55*g, Arrow);
//xaxis("$k_x$", YEquals(-gy), xmin=-1g, xmax=0, Arrow);
yaxis("$k_y$", XEquals(-1g), ymin=g*sqrt(3)/2, ymax=1.65*g, Arrow);
//yaxis("$k_y$", XEquals(-1g), ymin=-gy, ymax=0, Arrow);

limits(z(-1,0) - (0.4kf,1.2kf),
       z(3,2) + (-0.2kf,1.2kf), Crop);
